...
function openEditModal(modalData) {
  let field2Title,
    firstInput,
    title,
    subTitle,
  ...
  if (pageType === "pots") {
    ...
    ({ title, subTitle, modalIdValue, modalName, input2Value, modalTheme, modalColorName, colorAnimation, buttonText } = config);
  ...
  closeButton.addEventListener("click", () => {
    ...
    const editAddModal = document.querySelector("#edit-modal");
  ...
  submitButton.addEventListener("click", () => {
    ...
    if (canSubmit) {
      ...
      const modalId = document.querySelector("#edit-modal").querySelector("[data-id]").getAttribute("data-id");
  ...
  const wrapper = document.querySelector("#edit-modal");
...
function openAddModal(data) {
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    `
        <div id="add-modal" class="animate-fade-in z-2 fixed inset-0 bg-[rgb(0,0,0,0.5)] flex justify-center items-center">
          <div data-id="${modalIdValue}" class="bg-[#FFF] w-[335px] md:w-[560px] rounded-[12px] flex flex-col gap-[20px] p-[32px]">
            <div class="w-full flex justify-between items-center">
              <h1 class="text-[#201F24] text-[20px] md:text-[32px] font-bold leading-[120%]">${title}</h1>
              <img data-name="close-button" src="../assets/images/icon-close-modal.svg" class="hover:cursor-pointer w-[25.5px] h-[25.5px]" />
  ...
  closeButton.addEventListener("click", () => {
    // declaring editAddModal and the close button
    const editAddModal = document.querySelector("#add-modal");
  ...
  // close editAddModal on outside click
  const wrapper = document.querySelector("#add-modal");
  const editAddModal = wrapper.querySelector("div");
  clickOutClose(editAddModal, "animate-fade-out", 200, wrapper);
}
