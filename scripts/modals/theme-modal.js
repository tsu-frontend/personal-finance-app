...
class ThemeModal {
    static open(chosenTheme, data, setChosenTheme) {
        const themeButton = document.querySelector("#input-3");
        if (themeButton.querySelector("#theme-modal"))
            return;
        ...
        `;
            ...
            .join("");
        themeButton.insertAdjacentHTML("beforeend", `<div id="theme-modal-wrapper" class="animate-theme-open cursor-auto max-h-[300px] [@media(900px>=height)]:max-h-[200px] [&::-webkit-scrollbar]:hidden overflow-y-auto rounded-[8px] bg-[#FFF] absolute left-[-1px] top-[64px] w-[calc(100%+2px)] shadow-[0px_4px_24px_0px_rgba(0,0,0,0.25)]">
        ...
        ThemeSelectHandler.handle(setChosenTheme);
        ClickOutClose.handle(themeModal, "animate-theme-close", 300);
    }
}
export { ThemeModal };
