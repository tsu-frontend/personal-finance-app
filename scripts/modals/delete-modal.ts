...
class DeleteModal {
  static open(data: any[], modalId: string, tableName: string): void {
    ...
    const delConfirmBtn = document.querySelector("#delete-confirm-button") as HTMLElement;
    ...
    delConfirmBtn.addEventListener("click", () => {
      ...
      setTimeout(() => {
        deleteModal.remove();
        const modal = document.querySelector(`[data-id="${modalId}"]`) as HTMLElement;
