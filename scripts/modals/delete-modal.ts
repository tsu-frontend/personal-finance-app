...
class DeleteModal {
  static open(data: any[], modalId: string, pageType: string): void {
      ...
      `<div id="wrapper" class="animate-fade-in z-2 fixed inset-0 bg-[rgb(0,0,0,0.5)] flex justify-center items-center">
         <div id="delete-modal" class="bg-[#FFF] w-[335px] md:w-[560px] rounded-[12px] flex flex-col gap-[20px] p-[32px]">
           <div class="w-full flex justify-between">
             <h1 class="text-[#201F24] text-[20px] md:text-[32px] font-bold leading-[120%]">Delete ‘${
               pageType === "pots"
                 ? modalData.name
                 : pageType === "budgets"
                 ? modalData.category
           ...
           <p class="w-full text-[#696868] text-[14px] font-normal leading-[150%]">Are you sure you want to delete this ${
             pageType === "pots"
               ? "pot"
               : pageType === "budgets"
               ? "budget"
               : pageType
           }? This action cannot be reversed, and all the data inside it will be removed forever.</p>
    ...
    const delConfirmBtn = document.querySelector(
