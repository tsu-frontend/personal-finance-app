...
class CategoryModal {
  static open(
    ...
    const categoryButton = document.querySelector("#input-1") as HTMLElement;
    if (categoryButton.querySelector("#category-modal")) return;
    ...
    categoryButton.insertAdjacentHTML(
      "beforeend",
      `<div id="category-modal-wrapper" class="z-1 animate-theme-open cursor-auto max-h-[300px] [@media(900px>=height)]:max-h-[200px] [&::-webkit-scrollbar]:hidden overflow-y-auto rounded-[8px] bg-[#FFF] absolute left-[-1px] top-[64px] w-[calc(100%+2px)] shadow-[0px_4px_24px_0px_rgba(0,0,0,0.25)]">
    ...
    const categoryModalWrapper = document.querySelector(
    ...
    Array.from(categoryModal.children).forEach((category) => {
      if ((category as HTMLElement).tagName === "DIV") {
        category.addEventListener("click", () => {
          const chosenCategory = (category as HTMLElement).id;
          //   do validation here
          ...
          const input1 = document.querySelector("#input-1") as HTMLElement;
          if (input1) {
